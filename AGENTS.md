...
- **公開サイトと CI:** `book/_quarto.yml` の `project.resources` で `book/pdf/*.pdf` を `dist/pdf/` に同梱する. `index.qmd` の各週とナビバーの「演習 PDF」から相対パス `pdf/weekN-exercises.pdf` でリンクする. GitHub Actions は `make build` のあと `dist/pdf/` に 6 ファイルがあることを検査するので, **更新した PDF はリポジトリにコミットすること**（CI では LaTeX を入れず PDF は再生成しない）.

## ローカルでビルドしてから commit し, サイトを更新する

1. 原稿を編集する（主に `book/*.qmd`）.
2. **演習問題**（`week*-exercises.qmd`）を変えたら, LaTeX ありの環境で `make exercises-pdf` を実行し, `book/pdf/week1-exercises.pdf` ～ `week6-exercises.pdf` を更新する. 演習を触らない変更ならこの手順は省略してよい.
